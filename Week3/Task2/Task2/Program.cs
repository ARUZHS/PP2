...
{
    enum FSIMode
    { // Есть два состояния: папки и файлы. Нумеруем для удобства 
        DirectoryInfo = 1,
        FileInfo = 2
    }
    class Layer
    {
        ...
        public void Draw()
        {  // Метод Draw: будет рисовать
            Console.BackgroundColor = ConsoleColor.Black;
            Console.ForegroundColor = ConsoleColor.White;
            Console.Clear();
            for (int i = Left; i < Math.Min(Content.Length, Right + 1); ++i)
            { //  будем показывать только 21 контент
